...
# 5 "/opt/microchip/xc8/v2.20/pic/include/c99/inttypes.h" 2 3
...
# 11 "/opt/microchip/xc8/v2.20/pic/include/c99/inttypes.h" 2 3
...
void lcd_moveto_xy(uint8_t page, uint8_t column) {
  lcd_command(0xB0 | ((page) & 0x1F)); lcd_command((((column)+0) & 0x0F)); lcd_command(0x10 | ((((column)+0)>>4) & 0x0F));
...
void lcd_draw_image_P(uint8_t * progmem_image, uint8_t pages, uint8_t columns, uint8_t style) {
 uint8_t i,j = 0;
  uint8_t inv = (style & 4);
 while(j<pages && (lcd_get_position_page() < 8)) {
  for (i=0; i<columns && (lcd_get_position_column() < 102); i++) {
...
void lcd_draw_image_xy_P(uint8_t * progmem_image, uint8_t x, uint8_t y, uint8_t pages, uint8_t columns, uint8_t style) {
 ...
 if(offset)
  ...
  if(pages > 8 - lcd_get_position_page())
    pages = 8 - lcd_get_position_page();
...
void lcd_clear_area(uint8_t pages, uint8_t columns, uint8_t style) {
  ...
  if(pages > (max = 8 - lcd_get_position_page()))
    pages = max;
  if(columns > (max = 102 - lcd_get_position_column()))
    columns = max;
...
void lcd_init() {
# 325 "dogm-graphic.c"
    lcd_command(0x40 | ((0) & 0x3F));

      lcd_command(0xA0 | 1);
      lcd_command(0xC0 | 0x0);
    ...
    lcd_command(0xA6 | 0);
    lcd_command(0xA2 | 0);
    lcd_command(0x28 | ((7) & 0x07));
    lcd_command(0x20 | ((7) & 0x07));
    lcd_command(0x81); lcd_command(((0x9) & 0x3F));
    lcd_command(0xFA); lcd_command(0x10 | 0x80);

  lcd_clear_area_xy(8,102,0,0,0);
