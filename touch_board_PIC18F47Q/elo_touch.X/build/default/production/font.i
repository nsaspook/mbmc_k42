...
# 1 "/opt/microchip/xc8/v2.20/pic/include/c99/inttypes.h" 1 3
# 11 "/opt/microchip/xc8/v2.20/pic/include/c99/inttypes.h" 3
...
# 12 "/opt/microchip/xc8/v2.20/pic/include/c99/inttypes.h" 2 3

# 1 "/opt/microchip/xc8/v2.20/pic/include/c99/bits/alltypes.h" 1 3
# 14 "/opt/microchip/xc8/v2.20/pic/include/c99/inttypes.h" 2 3
...
static __attribute__((inline)) uint8_t lcd_get_position_page(void) {return lcd_current_page;}
static __attribute__((inline)) uint8_t lcd_get_position_column(void) {return lcd_current_column;}
...
# 40 "./font.h"
  extern const struct font_info font_fixed_8px;




  extern const struct font_info font_fixed_16px;




  extern const struct font_info font_proportional_16px;
  ...
  extern const struct font_info symbols_8px;




  extern const struct font_info symbols_16px;




  extern const struct font_info digits_24px;




  extern const struct font_info digits_32px;
# 119 "./font.h"
...
__attribute__((inline)) void lcd_set_font(FONT_P font, uint8_t style){
  global_font_select = font;
  global_font_style = style;
}
# 75 "font.c"
__attribute__((inline)) int font_data(FONT_P font) {
  int tmp;
  if (sizeof(tmp) == 2)
    tmp=&font->data;
  else
...
__attribute__((inline)) int font_widthtable(FONT_P font) {
  uint16_t tmp;
  if (sizeof(tmp) == 2)
...
__attribute__((inline)) uint8_t font_get_height_bytes(FONT_P font) {
  FONT_P tmp = font;
...
__attribute__((inline)) int16_t font_get_char_number(FONT_P font, char character) {
  FONT_P tmp = font;
...
__attribute__((inline)) uint8_t font_get_char_width(FONT_P font, char character) {
...
# 162 "font.c"
__attribute__((inline)) unsigned char double_bits(uint8_t part, char c) {
  uint8_t t = 0;
  if (part) c = c>>4;
  if (c & 0x08) t = 0xC0;
  if (c & 0x04) t |= 0x30;
...
uint8_t lcd_put_char(FONT_P font, uint8_t style, char character) {
  ...
  uint8_t row = 0;
    ...
    uint8_t hc = (style & 1)?1:0;

  uint8_t wc = (style & 2)?1:0;
  uint8_t ul = (style & 16)?0x80:0x00;
  uint8_t inv = (style & 4)?0xFF:0;
  uint8_t spc = (style & 32)?3:1;
  uint8_t tmp;
  ...
  if ((style & 8) && (lcd_get_position_column() + char_final_width > 102)) {
    lcd_moveto_xy((lcd_get_position_page()+char_final_height),(0));
  ...
  do {
    ...
    lcd_move_xy((1),(-char_final_width));
    } while (++row < char_final_height);


  lcd_move_xy((-char_final_height),(char_final_width));
